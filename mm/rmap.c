 ...
 * 将avc->anon_vma指向获得的vma(此vma可能是新建的，也可能是可以合并的vma的anon_vma)，
   avc->vma指向vma，并把avc加入到vma的anon_vma_chain中
 */
int anon_vma_prepare(struct vm_area_struct *vma)
{
  ...
  /* 如果此vma的anon_vma为空，则进行以下处理 */
     ...
     * 是否都为文件映射，除了(VM_READ|VM_WRITE|VM_EXEC|VM_SOFTDIRTY)其他标志位是否相同，
       如果为文件映射，前/后vma映射的文件位置是否正好等于vma映射的文件 + vma的长度
...
int anon_vma_fork(struct vm_area_struct *vma, struct vm_area_struct *pvma)
{
  ...
  /* 父进程的此vma没有anon_vma，直接返回 */
	 ...
	 */
  /* 这里开始先检查父进程的此vma是否有anon_vma，有则继续，而上面进行了判断，
     只有父进程的此vma有anon_vma才会执行到这里
	...
	struct vm_area_struct *vma, unsigned long address)
{
   ...
   * page->index存放此page在vma中的虚拟页框号，
     计算方法：page->index = ((address - vma->vm_start) >> PAGE_SHIFT) + vma->vm_pgoff;
   */
