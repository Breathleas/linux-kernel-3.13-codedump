					...
					struct per_cpu_pages *pcp)
{
  ...
  // 目前正在释放内存，将此清0，待回收过程随后回收时重新计数 
	...
	while (to_free) { // 连续释放指定数量的页面到伙伴系统
		 ...
		 */
    /* 
       这里是从MIGRATE_UNMOVABLE到MIGRATE_MOVABLE三个每CPU缓存链表中依次搜索一个可释放的页面，
       直到找到一个可以回收的缓存链表。 
    */
    do {
    ...
    // 本循环从最后一个页面缓存链表中释放多个页面到伙伴系统，越靠后的缓存链表，释放的页面数量越多
      ...
      // 取缓存链表的最后一个元素，要么是最早放到缓存中的页面(其硬件缓存的热度也最低)
      // ，要么就是"冷"页，不需要考虑硬件缓存
      ...
      // 将页面释放回伙伴系统。注意这里没有用migratetype变量，而是用页面属性中的migratetype。
      // 原因请参见英文注释
				  ...
				  int start_type, int fallback_type)
{
	...
	if (current_order >= pageblock_order / 2 || // 要分割的页面是一个大页面,则将整个页面全部迁移到当前迁移类型的链表中,这样可以避免过多的碎片
	    start_type == MIGRATE_RECLAIMABLE ||  // 目前分配的是可回收页面,这类页面有突发的特点,将页面全部迁移到可回收链表中,可以避免将其他迁移链表分割成太多的碎片
...
void free_hot_cold_page(struct page *page, int cold)
{
   ...
   */
	...
	if (migratetype >= MIGRATE_PCPTYPES) {  // 该页面目前不受每CPU的PCP链表管理
		if (unlikely(is_migrate_isolate(migratetype))) { // 该页面是从邻近的CPU管理区中调配过来的，仍然放到MIGRATE_ISOLATE中，不转化为可迁移的页面
  ...
  // 取得本CPU的页面缓存对象
	...
	if (cold)
    // 如果是冷页，则CPU缓存中还没有页面数据，将页面放入页面缓存链表的尾部
		list_add_tail(&page->lru, &pcp->lists[migratetype]);
	else
    // 否则放入列表的首部，这样后续的分配过程可以利用CPU缓存中的热缓存
			...
			int migratetype)
{
	unsigned long flags;
	struct page *page;
  // 如果分配参数指定了__GFP_COLD标志，则设置cold标志
...
again:
    ...
    // 这里需要关中断，因为内存回收过程可能发送核间中断，强制每个核从每CPU缓存中释放页面。
    // 而且中断处理函数也会分配单页。 
		local_irq_save(flags);
    // 取得本CPU的页面缓存对象
		pcp = &this_cpu_ptr(zone->pageset)->pcp;
    ...
    // 根据分配标志，确定使用哪一个水线
      ...
      // 运行到此，说明该管理区中内存不足，需要对该管理区进行回收
        ...
        // 回收了部分内存
				...
				if (zone_watermark_ok(zone, order, mark,
						classzone_idx, alloc_flags))
          // 回收的内存满足本次分配要求，跳转到try_this_zone
					goto try_this_zone;
...
try_this_zone:
    // 这个标签表示当前管理区中有足够的可用内存，试图在此管理区中分配内存
...
this_zone_full:
    // 这个标签标识当管理区中不能满足分配要求
		if (IS_ENABLED(CONFIG_NUMA))
      // 标记该zone已经满了，下次分配时将略过本管理区，避免浪费太多时间
	...
	int migratetype)
{
	 ...
	 * too large.
	 */
  // 检查order，避免出现大于等于MAX_ORDER的情况出现
	if (order >= MAX_ORDER) {
  ...
  */
  ...
  /**
    ...
    */
	...
	if (page)
		goto got_pg;
  // 将页面迁移标志设置为同步方式，这样第二次页面迁移都会使用同步方式
	sync_migration = true;
...
void __free_pages(struct page *page, unsigned int order)
{
    ...
    // 一般来说，应当在页面引用计数变为0时才会调用释放函数，这里进行引用计数判断是安全起见 
		if (order == 0)
      // 如果是释放单个页面，则释放到管理区的每CPU页面缓存中，以加快单页分配速度
